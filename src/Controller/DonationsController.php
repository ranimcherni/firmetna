...
public function demander(Request $request, Offre $offre, EntityManagerInterface $em): Response
{
    ...
    if ($qty <= 0 || $qty > $offre->getQuantite()) {
        $this->addFlash('danger', "Quantité indisponible.");
        return $this->redirectToRoute('app_donations');
    }
    ...
    // 4. On stocke l'ID en session pour déclencher le téléchargement auto en JS
    $request->getSession()->set('download_pdf', $demande->getId());

    $this->addFlash('success', 'Demande enregistrée ! Votre bon de don va être téléchargé.');
    return $this->redirectToRoute('app_donations');
}

/**
 * Route séparée pour générer le PDF uniquement
 */
#[Route('/download-pdf/{id}', name: 'app_donations_pdf_download')]
public function downloadPdf(Demande $demande): Response
{
    // Sécurité : vérifier que l'utilisateur est bien le demandeur
    ...
    //     // 2. Création de la demande (CORRECTION ICI : -> au lieu de .)
    //     $demande = new Demande();
    //     $demande->setOffre($offre);
    //     $demande->setDemandeur($user);
    //     $demande->setQuantiteDemandee($qty);
...
public function listByType(
    ...
    // Gérer l'ajout d'un don même quand on est dans une catégorie filtrée
    if ($form->isSubmitted() && $form->isValid()) {
        $photoFile = $form->get('photoFile')->getData();
        if ($photoFile) {
            $newFilename = uniqid().'.'.$photoFile->guessExtension();
